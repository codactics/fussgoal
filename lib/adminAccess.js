...
export function canAdminAccessTournament(session, tournament) {
  ...
  const assignedAdmins = Array.isArray(tournament.tournamentAdmins)
    ? tournament.tournamentAdmins
    : [];

  return (
    (Boolean(tournament.ownerUsername) && tournament.ownerUsername === session.username) ||
    assignedAdmins.some((admin) => admin?.username === session.username)
  );
}
...
export function buildTournamentAdminFilter(session) {
  ...
  return {
    $or: [
      ...
      { "tournamentAdmins.username": session.username },
    ],
  };
}
